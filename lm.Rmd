...
we now have all the unknowns to arrive at the solution. Lets plug the numbers in and compare it to the 95% confidence interval we obtained above. 
...
1) Here is the dataaset:
```{r}
dat_salary <-  data.frame("iq" = c(120, 110,  100, 135, 140),
                          "salary(m)" = c(2500, 2300, 2400, 3000, 2100))
```


2) And the model's output:
```{r}
summary(lm(dat_salary$salary.m. ~ dat_salary$iq))
```



```{r}
ggplot(dat_salary,  aes(x = iq, y = salary.m.)) +
