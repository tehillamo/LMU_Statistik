...
What does this mean? Lets say that we want to predict the price of a rented flat, given the number of rooms it has. In this case, our **independent variable** $y$ represent all the sampled prices of rented flats we collected (as we already know, this is only a sample of rented flats) and $x$ represent the **dependent variable** all the number of rooms associated with those flats. 

Lets say that I would like to know what is the predicted value of a flat, given that it has 3 rooms. In this case:

$\mathbb{E}[Y_{(\$flat)}|X_{(\#rooms = 3)} = \beta_{0} + \beta_{1} \times x_{1}$

$\beta_{0}$ is the intercept, which is the value of $Y$ when $X$ is zero.

$\beta_{1}$ is the slope of the line, which represents the change in $Y$ for every unit increase in $X$, 
...
<video width="500" height="300" controls><source src="img/CoordExmp.mp4" type="video/mp4"></video>
