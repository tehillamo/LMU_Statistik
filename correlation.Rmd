...
$$
cor_{xy} = \frac{1}{n} \times \frac{\sum_{i=1}^{n}(x_i - \bar{x})(y_i - \bar{y})}{s_{x} \times s_{y}}
$$

Wo:

- \( cor_{xy} \) der Korrelationskoeffizient zwischen \( x \) und \( y \) ist.

- \( x_i \) und \( y_i \) einzelne Vektoren sind.

- \( \bar{x} \) und \( \bar{y} \) die Mittelwerte von \( x \) und \( y \) sind.


- $s_{x}$ und $s_{y}$ die Standardabweichungen von \( x \) und \( y \) sind.
...
2. Ein Streudiagramm erstellen, um die Korrelation zwischen den zwei Vektoren zu beobachten: 
